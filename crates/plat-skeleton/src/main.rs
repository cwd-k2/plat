...
use plat_manifest::{Case, DeclKind, Declaration, Field, Language, Manifest, Op};
...
struct Config {
    ...
    default_types: HashMap<&'static str, &'static str>,
...
impl Config {
    fn layer_dir(&self, layer: Option<&str>) -> String {
        match layer {
            Some(ly) => self.layer_dirs.get(ly).cloned().unwrap_or_else(|| ly.to_string()),
    ...
    fn default_layer_dir(&self) -> String {
        match self.language {
            Language::Go => "domain".to_string(),
            Language::TypeScript => "domain".to_string(),
            Language::Rust => "domain".to_string(),
        }
    }
...
fn main() {
    ...
    let manifest: Manifest = serde_json::from_str(&manifest_text).unwrap_or_else(|e| {
    ...
    let cfg = Config {
        ...
        go_module: cli.module,
    };

    let files = generate(&cfg, &manifest);

    for (path, content) in &files {
        let full = cfg.output.join(path);
...
fn generate(cfg: &Config, manifest: &Manifest) -> Vec<(String, String)> {
    match cfg.language {
...
/// Convert PascalCase to kebab-case (for TypeScript file names).
...
fn go_returns(cfg: &Config, op: &Op) -> String {
    ...
    match (non_err.len(), has_err) {
        ...
        (1, true) => format!(
            " ({}, error)",
            cfg.resolve_type(&non_err[0].typ)
        ),
        (_, true) => {
        ...
        _ => {
            let types: Vec<String> = op.outputs.iter().map(|f| cfg.resolve_type(&f.typ)).collect();
            format!(" ({})", types.join(", "))
        }
    }
}
...
fn go_operation(cfg: &Config, manifest: &Manifest, decl: &Declaration) -> Vec<(String, String)> {
    ...
    out.push_str(&format!("package {pkg}\n"));
    out.push('\n');
    out.push_str(&format!("// {name} — generated from operation {}\n", decl.name));

    // Struct with dependency fields
    out.push_str(&format!("type {name} struct {{\n"));
    for dd in &dep_decls {
...
fn go_execute_params(cfg: &Config, decl: &Declaration) -> String {
    // Collect inputs from all ops of this operation declaration
...
fn go_execute_returns(cfg: &Config, decl: &Declaration) -> String {
    // Collect outputs from all ops of this operation declaration
    ...
    match (non_err.len(), has_err) {
        ...
        _ => {
            let types: Vec<String> = outputs.iter().map(|f| cfg.resolve_type(&f.typ)).collect();
            format!("({})", types.join(", "))
        }
    }
}
...
fn go_adapter(cfg: &Config, manifest: &Manifest, decl: &Declaration) -> Vec<(String, String)> {
    ...
    out.push_str(&format!("package {pkg}\n"));
    out.push('\n');
    out.push_str(&format!("// {name} — generated from adapter {}\n", decl.name));
...
fn ts_operation(cfg: &Config, manifest: &Manifest, decl: &Declaration) -> Vec<(String, String)> {
    ...
    // Collect inputs/outputs from ops
    let inputs: Vec<&Field> = decl.ops.iter().flat_map(|op| &op.inputs).collect();
...
fn ts_adapter(cfg: &Config, manifest: &Manifest, decl: &Declaration) -> Vec<(String, String)> {
    ...
    // Method stubs for implemented boundary
...
fn generate_rust(cfg: &Config, manifest: &Manifest) -> Vec<(String, String)> {
    let mut files = Vec::new();
    // Group declarations by layer for mod.rs generation
    ...
    // Generate mod.rs for each layer
...
fn rust_trait_method(cfg: &Config, op: &Op) -> String {
    ...
    for f in &op.inputs {
        params.push(format!("{}: {}", snake(&f.name), cfg.resolve_type(&f.typ)));
    }
