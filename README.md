...
各 example はアーキテクチャ定義を `Arch/` モジュール階層で構造化し、生成物を `dist/` に出力する:
...
examples/go-clean-arch/
  ...
  src/                    -- Go 実装コード
  plat-verify.toml        -- plat-verify 設定
```
...
Requires GHC >= 9.6 (recommended 9.12+). `OverloadedStrings` のみ必須。
