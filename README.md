...
plat は現在 Hackage 未公開のため、`cabal.project` で git 依存として指定する:
...
| File | Content |
|------|---------|
| `check.txt` | Haskell 側の検証結果 (レイヤー依存違反、未定義型など) |
| `manifest.json` | 構造定義 (plat-verify の入力) |
| `architecture.md` | アーキテクチャドキュメント (Markdown) |
| `architecture.mmd` | 依存関係ダイアグラム (Mermaid) |

Go ターゲットの場合は `Plat.Target.Go` でスケルトンコードも生成できる:

```haskell
import qualified Plat.Target.Go as Go

let cfg = Go.defaultConfig "github.com/my-org/my-service"
mapM_ (\(fp, content) -> out ("dist/skeleton/" <> fp) content)
      (Go.skeleton cfg architecture)
```
...
```bash
plat-verify dist/manifest.json
```

出力例 (全チェック合格):

```
plat-verify: my-service (go)

All checks passed.

── Summary ────────────────────────────────────────
  0 error(s), 0 warning(s), 0 info
  declarations: 5 checked, 5 ok, 0 issues
```
...
[E001] ERROR Order  type "Order" not found in source
       source: src/domain/

[S001] ERROR PlaceOrder  missing field: "order"
       expected: Order
       source: src/usecase/place_order.go:5

── Summary ────────────────────────────────────────
  2 error(s), 0 warning(s), 0 info
  declarations: 5 checked, 3 ok, 2 issues
```

JSON 出力 (`--format json`) や severity フィルタ (`--severity warning`) も可能。
詳細なオプションは `plat-verify --help` を参照。

## API Overview

以下は plat の eDSL の主要な構成要素を示す:
