...
layer_deps = false                 # L0xx: レイヤー依存方向チェック (opt-in)
...
### R0xx: Compose (manifest 内部整合性)

| Code | Severity | Condition |
|------|----------|-----------|
| R003 | error | binding が参照する boundary/adapter が manifest に存在しない |
| R004 | warning | boundary に対応する binding が存在しない |

compose チェックは常に実行される (manifest 内部の整合性検証のため)。

### L0xx: Layer Dependencies (レイヤー依存方向, opt-in)

| Code | Severity | Condition |
|------|----------|-----------|
| L001 | error | 宣言が許可されていない layer の型を参照している |

manifest の `layers[].depends` で定義された依存方向に基づき、各宣言の field 型、op パラメータ、needs、implements、injects から抽出した型参照が許可された layer にあるかをチェックする。

- 型参照の解析: `T?` → `T`, `List<T>` → `T`, ビルトイン型はスキップ
- manifest に存在しない型参照 (外部型・プリミティブ) はスキップ
- 同一 layer 内の参照は常に許可

```toml
[checks]
layer_deps = true  # opt-in, default false
```
...
- **TypeScript: `type` alias**: `type Foo = { ... }` 形式のオブジェクト型エイリアスは `interface` として抽出される。union / primitive alias はスキップ
