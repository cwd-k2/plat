...
2. Smart constructor: core コンストラクタ + `meta "plat-newext:kind" "..."` のラッパー
3. Query helper: `lookupMeta` ベースの述語
4. ルール (任意): `newextRules :: [SomeRule]`
5. `plat-hs.cabal` の `exposed-modules` に追加
6. Meta 命名規約: `plat-{ext}:{key}` / `plat-{ext}:{key}:{subkey}`
